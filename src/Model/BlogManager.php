...
class BlogManager extends AbstractManager
{
    ...
    {
        $statement = $this->pdo->prepare(" INSERT INTO " . self::TABLE . " (`title`, `date`, `name`, `text`) VALUES (:title,:date,:name,:text)");
    ...
    {
        $statement = $this->pdo->prepare("UPDATE " . self::TABLE . " SET `title`=:title, `date`=:date, `name`=:name, `text`=:text WHERE id=:id");
